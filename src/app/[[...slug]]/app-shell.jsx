...
function usePersistentState(key, defaultValue = null) {
  const [, bump] = useReducer((count) => count + 1, 0);
  const stored = readSession(key);
  const value = stored === null ? defaultValue : stored;
...
export default function Home() {
  ...
  // Filtro de unidade do topo: persistido para sobreviver a F5 (a navegação em si já
  // não zera estado — o shell vive no layout do segmento, que o App Router preserva).
  const [branchId, setBranchId] = usePersistentState("fdk:branchId", "");
  ...
  return <div className="min-h-screen" style={themeStyle}>
    ...
    <main key={view} className={mainClassName}>
      ...
      {/* Em edição, espera a lista de usuários carregar antes de montar o form: o initializer
          do UserFormView captura o usuário na montagem — montar com a lista vazia (ex.: F5
          direto nesta rota) abriria o form em branco como se fosse cadastro novo. */}
      {view === "users-form" && can("users", "read") && (userEditId && !users.length ? <Skeleton className="h-[420px]" /> : <UserFormView userId={userEditId} users={users} branches={data.branches} assets={data.assets} profiles={profiles} canGrantAllBranches={data.currentUser?.role === "ADMIN" && Boolean(data.permissions?.canViewAllBranches)} onCreate={createUser} onSave={saveUser} onCancel={closeUserForm} />)}
      ...
      {view === "settings-types" && can("ticket_types", "read") && <SettingsTypesView catalog={catalog} onToggleType={toggleTicketType} onNew={() => { setFormDraft(null); setView("settings-types-form"); }} onEdit={(type) => { setFormDraft(type); setView("settings-types-form"); }} onConfigureWorkflow={(type) => { setFormDraft(type); setView("settings-types-workflow"); }} onImported={reloadCatalog} />}
      ...
      {view === "settings-locations" && can("locations", "read") && <SettingsLocationsView key={`locations-${branchId}`} branches={branches.length ? branches : data.branches} branchId={branchId} />}
      ...
      {view === "recurring-tickets" && can("recurring_tickets", "read") && <RecurringTicketsView key={`${listRefreshKey}-${branchId}`} branches={branches.length ? branches : data.branches} branchId={branchId} canConfigure={can("recurring_tickets", "create") || can("recurring_tickets", "update")} onNew={() => { setFormDraft(null); setView("recurring-tickets-form"); }} onEdit={(item) => { setFormDraft(item); setView("recurring-tickets-form"); }} />}
