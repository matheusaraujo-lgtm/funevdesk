...
function UserSidePanel({ user, onClose, onEdit, onResetPassword, onToggle, onDelete, currentUserId }) {
  return <Card className="h-fit gap-0 rounded-2xl border-0 py-0 shadow-none ring-1 ring-foreground/10 lg:sticky lg:top-24">
    ...
    <div className="space-y-4 p-5">
      <div className="flex items-center gap-3"><Avatar className="size-10">{user.avatarUrl && <AvatarImage src={user.avatarUrl} alt={user.name} />}<AvatarFallback>{initials(user.name)}</AvatarFallback></Avatar><div><p className="font-medium">{user.name}</p><p className="flex items-center gap-1 text-xs text-muted-foreground"><Mail className="size-3" />{user.email}</p></div></div>
      <Separator />
